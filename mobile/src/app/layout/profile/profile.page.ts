  ...
  guardar() {
    this.userDataService.put(this.user).then( r => {
      ...
      if (this.cambiandoClaves && this.clavesCoinciden) {
        ...
        this.presentToastWithOptions('Datos guardados satisfactoriamente. Cierre sesión para visualizar los cambios.', 'middle', 'Aceptar', 'success','Guardado','save-outline','start').then(
          r_success => {
