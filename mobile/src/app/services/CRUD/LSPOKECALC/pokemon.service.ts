...
export class PokemonService {
   ...
   url = environment.api_lspokecalc + 'pokemon/';
   ...
   post(pokemon: Pokemon): Promise<any> {
      return this.http.post(this.url, JSON.stringify(pokemon), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(pokemon: Pokemon): Promise<any> {
      return this.http.put(this.url, JSON.stringify(pokemon), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
