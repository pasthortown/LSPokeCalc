...
export class NatureService {
   ...
   url = environment.api_lspokecalc + 'nature/';
   ...
   post(nature: Nature): Promise<any> {
      return this.http.post(this.url, JSON.stringify(nature), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(nature: Nature): Promise<any> {
      return this.http.put(this.url, JSON.stringify(nature), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
