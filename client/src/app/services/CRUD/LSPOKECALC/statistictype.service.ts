...
export class StatisticTypeService {
   ...
   url = environment.api_lspokecalc + 'statistictype/';
   ...
   post(statistictype: StatisticType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(statistictype), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(statistictype: StatisticType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(statistictype), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
